...
p = {
    ...
    "extraartists(./extraartists/artist)": [
        {
            "id": "number(id)",
            "name": "./name",
            "join?": "./join",
...
p = parslepy.Parselet(p)

class LxmlReleaseHandler(object):
    def parse(self, f):
        for cnt, (t, element) in enumerate(lxml.etree.iterparse(f, tag="release"), start=1):
            if element.attrib.get('id'):
                ...
                if False:
                    release.id = int(element.attrib.get('id', 0))
                    release.master_id = int(element.attrib.get('master_id', 0))
                    release.status = element.attrib.get('status')
                    for att, xp in (('title', './title'),
                                    ('country', './country'),
                                    ('data_quality', './data_quality'),
                                    ('released', './released')
                                    ):
                        t = element.find(xp)
                        if t is not None:
                            setattr(release, att, t.text)
                    for att, xp in (('genres', './genres/genre'),
                                    ('styles', './styles/style'),
                                    ):
                        elems = element.find(xp)
                        if elems is not None:
                            l = []
                            for elem in elems:
                                l.append(elem.text)
                        setattr(release, att, l)

                    for trackelem in element.xpath('./tracklist/track'):
                        track = model.Track()
                        for att, xp in (('title', './title'),
                                        ('position', './position'),
                                        ('duration', './duration'),
                            ...
                            t = trackelem.find(xp)
                            if t is not None:
                                setattr(track, att, t.text)
                        release.tracklist.append(track)

                    for labelelem in element.xpath('./labels/label'):
                        lbl = model.ReleaseLabel()
                        for att, elatt in (('catno', 'catno'),
                                        ('name', 'name'),
                                        ):
                            setattr(lbl, att, labelelem.attrib.get(xp))
                        release.labels.append(lbl)

                    for videoelem in element.xpath('./videos/video'):
                        vid = model.Video()
                        for att, elatt in (('duration', 'duration'),
                                           ('src', 'uri'),
                                        ):
                            setattr(vid, att, videoelem.attrib.get(elatt))
                        release.videos.append(vid)
