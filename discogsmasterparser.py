...
class LxmlMasterHandler(object):
    def parse(self, f):
        for cnt, (t, element) in enumerate(lxml.etree.iterparse(f, tag="master"), start=1):
            if element.attrib.get('id'):
                ...
                #continue
                master = model.Master()
                master.id = int(element.attrib.get('id', 0))
