...
class LxmlArtistHandler(object):
    def parse(self, f):
        for cnt, (t, element) in enumerate(lxml.etree.iterparse(f, tag="artist"), start=1):
            if element.xpath('./id'):
                ...
                for att, xp in (('id', './id'),
                    ...
                    t = element.find(xp)
                    if t is not None:
                        setattr(artist, att, t.text)
                artist.id = int(artist.id)
                for att, xp in (('urls', './urls/url'),
                                ('namevariations', './namevariations/name'),
