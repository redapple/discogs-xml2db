...
def parseArtists(parser, exporter):
    ...
    from discogsartistparser import LxmlArtistHandler
    artistparser = LxmlArtistHandler()
    if artist_file.endswith(".gz"):
        with gzip.open(artist_file) as f:
            artistparser.parse(f)
    else:
        artistparser.parse(artist_file)
    return
...
def parseLabels(parser, exporter):
    ...
    from discogslabelparser import LxmlLabelHandler
    labelparser = LxmlLabelHandler()
    if label_file.endswith(".gz"):
        with gzip.open(label_file) as f:
            labelparser.parse(f)
    else:
        labelparser.parse(label_file)
    return
...
def parseReleases(parser, exporter):
    ...
    from discogsreleaseparser import LxmlReleaseHandler
    releaseparser = LxmlReleaseHandler()
    if release_file.endswith(".gz"):
        with gzip.open(release_file) as f:
            releaseparser.parse(f)
    else:
        releaseparser.parse(release_file)
    return
...
def parseMasters(parser, exporter):
    ...
    from discogsmasterparser import LxmlMasterHandler
    masterparser = LxmlMasterHandler()
    if master_file.endswith(".gz"):
        with gzip.open(master_file) as f:
            masterparser.parse(f)
    else:
        masterparser.parse(master_file)
    return
